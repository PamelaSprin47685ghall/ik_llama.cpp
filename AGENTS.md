目前代码中权重 hadamard 变换的部分造成乱码。请你修复。相关原则是，优先保证正确性，然后如果某些情况 Hadamard 可抵消，尽量抵消。权重 k v 三者 的 hadamard 可以独立开启，共 2x2x2=8 情况，全部要正确。也许量化过程也不对，但尽量先用现有量化结果跑通，最后再修量化过程。Q4_0_HADAMARD khad vhad 三者就是唯一真理，连根拔除所有别的开关。请你一次性改代码到绝对有信心对，再调试。请你完整审计所有 hadamard 和 un-hadamard 全链路，所有分支的代码。
